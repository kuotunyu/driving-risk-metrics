...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            np.zeros((2, 2), dtype=np.uint8),
            0.0,
            r"^image must have shape \[height, width, 3\]$",
        ),
        (
            np.zeros((2, 2, 3), dtype=np.float32),
            np.zeros((2, 2), dtype=np.uint8),
            0.0,
            r"^image must have uint8 dtype$",
        ),
        (
            np.zeros((2, 2, 3), dtype=np.uint8),
            np.zeros((2, 2, 1), dtype=np.uint8),
            0.0,
            r"^mask must have shape \[height, width\]$",
        ),
        (
            np.zeros((2, 2, 3), dtype=np.uint8),
            np.zeros((2, 2), dtype=np.float32),
            0.0,
            r"^mask must have uint8 dtype$",
        ),
        (
            np.zeros((2, 3, 3), dtype=np.uint8),
            np.zeros((2, 2), dtype=np.uint8),
            0.0,
            r"^image and mask spatial shapes must match$",
        ),
        (
            np.zeros((2, 2, 3), dtype=np.uint8),
            np.zeros((2, 2), dtype=np.uint8),
            -0.1,
            r"^flip_draw must be finite and within \[0, 1\]$",
        ),
        (
            np.zeros((2, 2, 3), dtype=np.uint8),
            np.zeros((2, 2), dtype=np.uint8),
        ...
        (
            ...
            0.0,
            r"^image and mask spatial dimensions must be positive$",
        ),
        (
            np.zeros((1, 3, 3), dtype=np.uint8),
            np.zeros((1, 3), dtype=np.uint8),
            0.0,
            r"^input aspect ratio does not fit the 512x1024 canvas$",
        ),
        (
            np.zeros((2, 2, 3), dtype=np.uint8),
            np.zeros((2, 2), dtype=np.uint8),
